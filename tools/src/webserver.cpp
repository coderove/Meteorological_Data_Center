...
int main(int argc,char *argv[])
{
    ...
    if (!logfile.Open(argv[1], "a+")) { printf("logfile.Open(%s) failed.\n", argv[1]); return -1; }
    ...
    // 初始化数据库连接池。
    if (!oraconnpool.init(starg.connstr, starg.charset, 10, 50))
    {
    ...
    {
        // 创建检查数据库连接池的线程。
        if (pthread_create(&checkpoolid,nullptr,checkpool,0)!=0)
        {
    ...
    for (int ii=0;ii<10;ii++)
    {
        ...
        if (pthread_create(&stpthinfo.pthid,nullptr,thmain,(void *)(long)ii)!=0)
        {
            logfile.Write("pthread_create() failed.\n"); return -1;
        }
    ...
    // 创建监控线程。
    if (pthread_create(&checkpthid,nullptr,checkthmain,(void *)0)!=0)
    {
        logfile.Write("pthread_create() failed.\n"); return -1;
    }
    ...
    pthread_cleanup_push(thcleanup,arg);  // 线程清理函数。
        ...
        while (true)
        {
            ...
            while (sockqueue.size()==0)
            {
                ...
                vthid[pthnum].atime=time(nullptr);        // 更新当前线程的活动时间。
            }
...
{
    printf("\n----------------------------------------------------------------------------------------\n");
    printf("[Using ] :./webserver_ logfilename xmlbuffer\n");
    printf("----------------------------------------------------------------------------------------\n");
    printf("[Sample] :./procctl 10 ./webserver /project/log/idc/webserver.log \"<connstr>scott/tiger@snorcl11g_132</connstr><charset>Simplified Chinese_China.AL32UTF8</charset><port>8080</port>\"\n");
    printf("----------------------------------------------------------------------------------------\n");
    printf("参数说明:\n\n");
