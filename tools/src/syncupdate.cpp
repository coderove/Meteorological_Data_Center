...
* File name     : syncupdate.cpp
* Version       : v1.0
* Author        : starrove 
* Date          : 2022/7/20 8:57.
* Description   : 本程序是数据中心的公共功能模块，采用刷新的方法同步MySQL数据库之间的表
**************************************************/
...
{
    printf("\n----------------------------------------------------------------------------------------\n");
    printf("[Using ] :./syncupdate logfilename xmlbuffer\n\n");
    printf("----------------------------------------------------------------------------------------\n");
    printf("[Sample] :./procctl 10 ./syncupdate /project/log/idc/syncupdate_MDCTCODE2.log \"<localconnstr>192.168.18.133,root,123456,mdcsql,3306</localconnstr><charset>utf8</charset><fedtname>LK_MDCTCODE1</fedtname><localtname>CODE_MDCTCODE2</localtname><remotecols>stand_id,stand_name,prov_name,lat,lon,height/10,upttime,keyid</remotecols><localcols>stand_id,st_name,prov_name,lat,lon,height,upttime,keyid</localcols><synctype>1</synctype><timeout>50</timeout><pname>syncupdate_MDCTCODE2</pname>\"\n\n");
    // 因为测试的需要，xmltodb程序每次会删除LK_MDCTCODE1中的数据，全部的记录重新入库，keyid会变。
    // 所以以下脚本不能用keyid，要用stand_id，用keyid会出问题，可以试试。
    printf("          ./procctl 10 ./syncupdate /project/log/idc/syncupdate_MDCTCODE3.log \"<localconnstr>192.168.18.133,root,123456,mdcsql,3306</localconnstr><charset>utf8</charset><fedtname>LK_MDCTCODE1</fedtname><localtname>CODE_MDCTCODE3</localtname><remotecols>stand_id,stand_name,prov_name,lat,lon,height/10,upttime,keyid</remotecols><localcols>stand_id,stand_name,prov_name,lat,lon,height,upttime,keyid</localcols><where>where stand_id like '54%%%%'</where><synctype>2</synctype><remoteconnstr>192.168.18.133,root,123456,mdcsql,3306</remoteconnstr><remotetname>CODE_MDCTCODE1</remotetname><remotekeycol>stand_id</remotekeycol><localkeycol>stand_id</localkeycol><maxcount>10</maxcount><timeout>50</timeout><pname>syncupdate_MDCTCODE3</pname>\"\n");
    printf("          ./procctl 10 ./syncupdate /project/log/idc/syncupdate_MDCMIND2.log \"<localconnstr>192.168.18.133,root,123456,mdcsql,3306</localconnstr><charset>utf8</charset><fedtname>LK_MDCMIND1</fedtname><localtname>CODE_MDCMIND2</localtname><remotecols>stand_id,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid</remotecols><localcols>stand_id,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid</localcols><where>where ddatetime>timestampadd(minute,-120,now())</where><synctype>2</synctype><remoteconnstr>192.168.18.133,root,123456,mdcsql,3306</remoteconnstr><remotetname>CODE_MDCMIND1</remotetname><remotekeycol>keyid</remotekeycol><localkeycol>keyid</localkeycol><maxcount>300</maxcount><timeout>50</timeout><pname>syncupdate_MDCTMIND2</pname>\"\n");
    printf("----------------------------------------------------------------------------------------\n");
    printf("参数说明:\n\n");
