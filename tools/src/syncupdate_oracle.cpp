...
* File name     : syncupdate_oracle.cpp
* Version       : v1.0
* Author        : starrove 
* Date          : 2022/7/21 9:11.
* Description   : 本程序是数据中心的公共功能模块，采用刷新的方法同步Oracle数据库之间的表
**************************************************/
...
bool _syncupdate() {
    ...
    if (starg.synctype == 1) {
        ...
        stmtins.prepare("insert into %s(%s) select %s from %s %s", starg.localtname, starg.localcols, starg.remotecols,
                        starg.lnktname, starg.where);
    ...
    stmtins.prepare("insert into %s(%s) select %s from %s where %s in (%s)", starg.localtname, starg.localcols,
                    starg.remotecols, starg.lnktname, starg.remotekeycol, bindstr);
    for (int ii = 0; ii < starg.maxcount; ii++) {
...
void _help() {
    printf("\n----------------------------------------------------------------------------------------\n");
    printf("[Using ] :./syncupdate_oracle logfilename xmlbuffer\n\n");
    printf("----------------------------------------------------------------------------------------\n");
    printf("[Sample] :./procctl 10 ./syncupdate_oracle /project/log/idc/syncupdate_oracle_MDCTCODE2.log \"<localconnstr>scott/tiger@snorcl11g_130</localconnstr><charset>Simplified Chinese_China.AL32UTF8</charset><lnktname>CODE_MDCTCODE1@db132</lnktname><localtname>CODE_MDCTCODE2</localtname><remotecols>stand_id,stand_name,prov_name,lat,lon,height,upttime,keyid</remotecols><localcols>stand_id,stand_name,prov_name,lat,lon,height,upttime,keyid</localcols><synctype>1</synctype><timeout>50</timeout><pname>syncupdate_oracle_MDCTCODE2</pname>\"\n");
    // 因为测试的需要，xmltodb程序每次会删除LK_MDCTCODE1中的数据，全部的记录重新入库，keyid会变。
    // 所以以下脚本不能用keyid，要用stand_id，用keyid会出问题，可以试试。
    printf("          ./procctl 10 ./syncupdate /project/log/idc/syncupdate_oracle_MDCTCODE3.log \"<localconnstr>scott/tiger@snorcl11g_130</localconnstr><charset>Simplified Chinese_China.AL32UTF8</charset><lnktname>CODE_MDCTCODE1@db132</lnktname><localtname>CODE_MDCTCODE3</localtname><remotecols>stand_id,stand_name,prov_name,lat,lon,height,upttime,keyid</remotecols><localcols>stand_id,stand_name,prov_name,lat,lon,height,upttime,keyid</localcols><where>where stand_id like '54%%%%'</where><synctype>2</synctype><remoteconnstr>qxidc/qxidcpwd@snorcl11g_132</remoteconnstr><remotetname>CODE_MDCTCODE1</remotetname><remotekeycol>stand_id</remotekeycol><localkeycol>obtid</localkeycol><maxcount>10</maxcount><timeout>50</timeout><pname>syncupdate_oracle_MDCTCODE3</pname>\"\n");
    printf("          ./procctl 10 ./syncupdate /project/log/idc/syncupdate_oracle_MDCMIND2.log \"<localconnstr>scott/tiger@snorcl11g_130</localconnstr><charset>Simplified Chinese_China.AL32UTF8</charset><lnktname>CODE_MDCMIND1@db132</lnktname><localtname>CODE_MDCMIND2</localtname><remotecols>stand_id,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid</remotecols><localcols>stand_id,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid</localcols><where>where ddatetime>sysdate-0.04</where><synctype>2</synctype><remoteconnstr>qxidc/qxidcpwd@snorcl11g_132</remoteconnstr><remotetname>CODE_MDCMIND1</remotetname><remotekeycol>keyid</remotekeycol><localkeycol>keyid</localkeycol><maxcount>300</maxcount><timeout>50</timeout><pname>syncupdate_oracle_MDCMIND2</pname>\"\n");
    printf("----------------------------------------------------------------------------------------\n");
    printf("参数说明:\n\n");
    ...
    printf("where         :同步数据的条件，为空则表示同步全部的记录，填充在delete本地表和select dblink表\n"\
