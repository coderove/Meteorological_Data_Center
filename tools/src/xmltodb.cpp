...
* File name     : xmltodb.cpp
* Version       : v1.0
* Author        : starrove 
* Date          : 2022/7/19 9:12.
* Description   : 本程序是数据中心的公共功能模块，用于把xml文件入库到MySQL的表中。
**************************************************/
...
bool _xmltodb()
{
    ...
    while (true)
    {
        ...
        while (true)
        {
            ...
            {
                if (iret==1) logfile.WriteEx("失败:没有配置入库参数。\n");
                if (iret==2) logfile.WriteEx("失败:待入库的表[%s]不存在。\n",stxmltotable.tname);
                if (iret==5) logfile.WriteEx("失败:待入库的表[%s]字段数太多。\n",stxmltotable.tname);
