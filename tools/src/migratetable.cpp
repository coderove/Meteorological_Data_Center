...
* File name     : migratetable.cpp
* Version       : v1.0
* Author        : starrove 
* Date          : 2022/7/20 14:13.
* Description   : 本程序是数据中心的公共功能模块，用于迁移表中的数据。
**************************************************/
...
int main(int argc,char *argv[])
{
    ...
    if (conn2.connecttodb(starg.connstr,nullptr) != 0)
    {
...
{
    printf("\n----------------------------------------------------------------------------------------\n");
    printf("[Using ]:./migratetable logfilename xmlbuffer\n");
    printf("----------------------------------------------------------------------------------------\n");
    printf("[Sample]:./procctl 3600 ./migratetable /project/log/idc/migratetable_MDCMIND.log \"<connstr>127.0.0.1,root,123456,mdcsql,3306</connstr><srctname>CODE_MDCMIND</srctname><dsttname>CODE_MDCMIND_HIS</dsttname><keycol>keyid</keycol><where>where ddatetime<timestampadd(minute,-120,now())</where><starttime>01,02,03,04,05,13</starttime><maxcount>300</maxcount><timeout>120</timeout><pname>migratetable_MDCMIND</pname>\"\n");
    printf("----------------------------------------------------------------------------------------\n");
    printf("参数说明:\n\n");
