...
* File name     : tcpputfiles.cpp
* Version       : v1.0
* Author        : starrove 
* Date          : 2022/7/16 10:13.
* Description   : TCP文件上传模块 --客户端
**************************************************/
...
bool ActiveTest() {
    ...
    // logfile.Write("发送：%s\n",strsendbuffer);
    if (!TcpClient.Write(strsendbuffer)) return false; // 向服务端发送请求报文。
...
bool _tcpputfiles() {
    ...
    if (!Dir.OpenDir(starg.clientpath, starg.matchname, 10000, starg.andchild)) {
        logfile.Write("[tcpputfiles _tcpputfiles] Dir.OpenDir(%s) 失败。\n", starg.clientpath);
        return false;
    }

    int delayed = 0;        // 未收到对端确认报文的文件数量。  epoll I/O复用 异步上传
    ...
    while (true) {
        ...
        if (!TcpClient.Write(strsendbuffer)) {
            logfile.Write("[tcpputfiles _tcpputfiles] TcpClient.Write() 失败.\n");
            return false;
        }
        ...
        while (delayed > 0) {
            memset(strrecvbuffer, 0, sizeof(strrecvbuffer));
            if (!TcpRead(TcpClient.m_connfd, strrecvbuffer, &buflen, -1)) break;
    ...
    while (delayed > 0) {
        memset(strrecvbuffer, 0, sizeof(strrecvbuffer));
        if (!TcpRead(TcpClient.m_connfd, strrecvbuffer, &buflen, 10)) break;
        // logfile.Write("strrecvbuffer=%s\n",strrecvbuffer);
...
bool SendFile(const int sockfd, const char *filename, const int filesize) {
    ...
    char buffer[1000];     // 存放读取数据的buffer。  单次读取字节数
    int totalbytes = 0;    // 从文件中已读取的字节总数。
    FILE *fp = nullptr;

    // 以"rb"的模式打开文件。还有二进制的文件
...
void _help() {
    printf("\n----------------------------------------------------------------------------------------\n");
    printf("[Using  ]       :./tcpputfiles logfilename xmlbuffer\n");
    printf("----------------------------------------------------------------------------------------\n");
    printf("[Example]       :./procctl 20 ./tcpputfiles /project/log/idc/tcpputfiles_surfdata.log \"<ip>192.168.18.133</ip><port>5005</port><ptype>1</ptype><clientpath>/project/tempdata/tcp/surfdata1</clientpath><andchild>true</andchild><matchname>*.XML,*.CSV,*.JSON</matchname><srvpath>/project/tempdata/tcp/surfdata2</srvpath><timetvl>10</timetvl><timeout>50</timeout><pname>tcpputfiles_surfdata</pname>\"\n");
    printf("                 ./procctl 20 ./tcpputfiles /project/log/idc/tcpputfiles_surfdata.log \"<ip>192.168.18.133</ip><port>5005</port><ptype>2</ptype><clientpath>/project/tempdata/tcp/surfdata1</clientpath><clientpathbak>/project/tempdata/tcp/surfdata1bak</clientpathbak><andchild>true</andchild><matchname>*.XML,*.CSV,*.JSON</matchname><srvpath>/project/tempdata/tcp/surfdata2</srvpath><timetvl>10</timetvl><timeout>50</timeout><pname>tcpputfiles_surfdata</pname>\"\n");
    printf("----------------------------------------------------------------------------------------\n");
    printf("参数说明:\n\n");
