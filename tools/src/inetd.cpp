 ...
 * 参数格式
 * 监听端口  目标IP          目标端口
 * 3006    192.168.16.133  3306   //mysql数据库端口
 */
...
int main(int argc, char *argv[]) {
    if (argc != 3) {
        ...
        printf("[Sample]:./inetd /project/log/inetd.log /project/tools/ini/inetd.conf\n");
        printf("         ./procctl 5 ./inetd /tmp/inetd.log /etc/inetd.conf\n");
        return -1;
    }
    ...
    // 把代理路由参数加载到vroute容器。
    if (!loadroute(argv[2])) return -1;

    logfile.Write("加载代理路由参数成功(%d)。\n", vroute.size());

    // 初始化服务端用于监听的socket。
    for (auto & ii : vroute) {
        if ((ii.listensock = initserver(ii.listenport)) < 0) {
            logfile.Write("initserver(%d) failed.\n", ii.listenport);
            EXIT(-1);
        }

        // 把监听socket设置成非阻塞。
        fcntl(ii.listensock, F_SETFL, fcntl(ii.listensock, F_GETFD, 0) | O_NONBLOCK);
    }
...
bool loadroute(const char *inifile) {
    ...
    while (true) {
        ...
        if (!File.FFGETS(strBuffer, 200)) break;
