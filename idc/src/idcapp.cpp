...
* File name     : idcapp.cpp
* Version       : v1.0
* Author        : starrove 
* Date          : 2022/7/17 19:53.
* Description   : 数据中心项目公用函数和类的实现
**************************************************/
...
bool CZHOBTMIND::SplitBuffer(char *strBuffer,bool bisxml)
{
    ...
    if (bisxml)
    {
        GetXMLBuffer(strBuffer,"stand_id",m_zhobtmind.stand_id,10);
    ...
    else
    {
        ...
        CmdStr.GetValue(0,m_zhobtmind.stand_id,10);
...
bool CZHOBTMIND::InsertTable()
{
    if (m_stmt.m_state==0)
    {
        ...
        m_stmt.prepare("insert into CODE_MDCMIND(stand_id,ddatetime,t,p,u,wd,wf,r,vis) values(:1,str_to_date(:2,'%%Y%%m%%d%%H%%i%%s'),:3,:4,:5,:6,:7,:8,:9)");
        m_stmt.bindin(1,m_zhobtmind.stand_id,10);
    ...
    if (m_stmt.execute()!=0)
    {
        ...
        if (m_stmt.m_cda.rc!=1062)
        {
            ...
            m_logfile->Write("m_stmt.execute() 失败 --> [%s] [%s]\n",m_stmt.m_sql,m_stmt.m_cda.message);
        }
